         ...
         <div class="kt-card h-full">
          <div class="kt-card-header">
           ...
           <div class="flex gap-5" wire:ignore>
            <select id="kt_select_year" class="kt-select min-w-[120px]" data-kt-select="true" data-kt-select-placeholder="Periodo" name="periodo">
             <option></option>
        ...
        <div class="lg:col-span-2">
         <div class="kt-card h-full">
          <div class="kt-card-header">
           <h3 class="kt-card-title">Últimas facturas generadas</h3>
            ...
            <thead>
             ...
             <tr>
              <td class="text-start">
               <a class="text-sm font-medium text-mono hover:text-primary" href="{{ route('invoices.show', $invoice) }}">FAC {{ str_pad($invoice->id, 3, '0', STR_PAD_LEFT) }}</a>
              </td>
              <td class="text-sm text-foreground text-start">{{ $invoice->client->name ?? 'Cliente Anónimo' }}</td>
              <td class="text-sm text-foreground text-end">${{ number_format($invoice->total_amount, 0, ',', '.') }}</td>
              <td class="text-end">
              ...
              <td class="text-sm text-foreground text-end">{{ \Carbon\Carbon::parse($invoice->due_date)->format('d M Y') }}</td>
              <td class="text-start">
          ...
          <div class="kt-card-footer justify-center">
           <a class="kt-link kt-link-underlined kt-link-dashed" href="{{ route('invoices.index') }}">Ver todas las facturas</a>
          </div>
         </div>
        </div>
        <div class="lg:col-span-1">
           ...
           <div class="flex flex-col gap-5">
            ...
            <div class="flex items-center justify-between gap-2.5">
             <div class="flex items-center gap-2.5">
              <div class="flex items-center justify-center relative text-2xl text-primary size-10 ring-1 ring-primary/20 bg-primary/5 rounded-full">{{ $client->initial }}</div>
              <div class="flex flex-col gap-0.5">
               <span class="leading-none font-medium text-sm text-mono">{{ $client->name }}</span>
               <span class="text-sm text-secondary-foreground">Saldo: ${{ number_format($client->debt, 0, ',', '.') }}</span>
              </div>
             </div>
