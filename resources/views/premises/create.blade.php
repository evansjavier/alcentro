...
@section('content')
    <div class="grid gap-5 lg:gap-7.5 xl:w-[38.75rem] mx-auto">
        ...
        <form class="grid gap-5" id="premises_form" method="POST" action="{{ route('premises.store') }}">
            @csrf
            <div class="kt-card pb-2.5">
                <div class="kt-card-header" id="basic_settings">
                    ...
                    <div class="flex items-center gap-2 text-sm text-secondary-foreground">
                        {{ __('Registra la información básica del local.') }}
                    </div>
                </div>
                <div class="kt-card-content grid gap-5">
                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                        ...
                        <div class="grow">
                            <input class="kt-input" id="code" name="code" value="{{ old('code') }}" placeholder="{{ __('Ej. LOC-101') }}" type="text">
                            @error('code')
                    ...
                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                        ...
                        <div class="grow">
                            <input class="kt-input" id="square_meters" name="square_meters" value="{{ old('square_meters') }}" placeholder="{{ __('Ej. 78.50') }}" type="number" step="0.01" min="0">
                            @error('square_meters')
                                <p class="text-sm text-destructive mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                        ...
                        <div class="grow">
                            <input class="kt-input" id="suggested_rent" name="suggested_rent" value="{{ old('suggested_rent') }}" placeholder="{{ __('Ej. 4500000') }}" type="number" step="0.01" min="0">
                    ...
                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                        ...
                        <div class="grow">
                            @php
                                $selectedStatus = old('status', 'available');
                            @endphp
            ...
            <div class="flex items-center justify-end gap-2">
                <a class="kt-btn kt-btn-outline" href="{{ route('dashboard') }}">
                    {{ __('Cancelar') }}
                </a>
                <button class="kt-btn kt-btn-primary" type="submit">
                    <span>{{ __('Guardar local') }}</span>
                </button>
            </div>
        </form>
    </div>
